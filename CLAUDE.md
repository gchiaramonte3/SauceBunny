...
| Session recordings | `~/Movies/Sauce Bunny/Sessions/<yyyy-mm-dd-hhmm>-<slug>/stage.mp4` — one folder per take, no index file (the directory IS the truth, like Frames). **`~/Movies`, not Documents, and that is load-bearing**: Documents is iCloud-synced for many users, and eviction has already been observed on this project breaking a git packfile, a DMG archive and the transcript scan. A 15 GB take is the worst thing to hand that mechanism |
...
| saucebunny-audio-analysis | Optional local-file sound evidence (system AVFoundation + SoundAnalysis, no capture); AI Summary adoption is not enabled yet | `npm run build:audio-analysis` (resource helper under `audio-runtime/`) |
...
| `pane-empty-contract` | The four drawer tabs' empty states share one title and one body class in base.css, and no pane defines its own `-empty-title`/`-body`/`-sub`. The box was shared and the words were not: switching tabs on an empty drawer changed the title's size (14 to 13px), weight (700 to 600) and colour, and each pane's classes were valid on their own |
| `inline-typography-contract` | No `style={{}}` in TSX carries a literal fontSize/fontWeight/lineHeight/letterSpacing/textTransform/fontFamily (a value computed from a prop or a table is the one reason to set these inline), and every `var(--x)` written in TSX names a property some stylesheet defines. Fourteen sites bypassed every typography contract, among them a `fontWeight: 500` no imported face renders and two properties defined nowhere whose fallbacks painted hot pink and #888 |
| `live-draw-contract` | Live telestration draws for the room and forgets. The live layer has no writer at all, the capture overlay opts out of drafting with `annotation={null}` + `onStroke`, expiry is local so one peer's fade cannot erase another's copy, and a clear does relay because the surface is shared. The capture surface is the SAME component the composer drafts a note with, told not to accumulate, so the promise is one prop deep and exactly what a later edit removes by accident |
| `spellcheck-contract` | Spell-check is enabled by the `WebContinuousSpellCheckingEnabled` user default, which the app sets once at startup and only when the user has never chosen, never by an HTML attribute. No comment may credit `lang` with it, and the comment box may not pin a language (it would override NSSpellChecker's language detection for a reviewer writing French). The opposite belief survived months here and got stronger with each restatement - a comment, then a hedged comment, then a commit message, then HAND-TEST, then the CHANGELOG - because nothing ever tested it; a WKWebView probe showed an attribute-free textarea underlines identically |
...
| `stage-overlay-contract` | Live reactions render inside `.cp-monitor`, the box sized to the video, not beside it. They were a sibling of Monitor and Transport in a `position: static` wrapper, so `absolute` resolved against a far ancestor spanning the whole column and `bottom: 90px` measured up from below the transport: a clap surfaced on the TIMECODE FIELD. Nothing in either file looked wrong on its own; the defect lived only in the relationship between the JSX and the CSS |
| `screening-record-contract` | Every exported screening UPDATER has a production caller, and both roles record. All four screening defects were one shape: declared, indexed, rendered, never written. `participants` was on the type, in index.json and on the shelf with no assignment anywhere, so every session read "0 people" and the `participants.length > 1` clause was unreachable; `markWatched` had no caller at all. Note the scan strips imports AND comments first: without that, deleting a call still matched the import list, and deleting the import still matched a comment explaining the bug |
| `session-scope-contract` | A note points AT the session; a session never holds a note. Comments carry an optional `sessionId`/`segmentId` stamped once at creation (the one idea worth taking from the W3C Web Annotation Data Model - its `scope`, not its vocabulary), and the screening carries comment IDS and nothing else. The acid test is structural here: opening a source solo, with no screening file present, must still show every note made about it in a session |
| `range-adopt-contract` | A range NOTE (shared, on every peer's timeline, exported as an NLE duration marker) can be adopted into this machine's own clip marks, which stay local. Both halves were built and neither was wrong; the verb between them was missing, because `onMarkRange`/`onQueueRange` reached TranscriptViewer and not the sibling ReviewPanel. Invisible in either component alone, which is how it survived. Also pins that co-review still never learns a clip mark, since broadcasting the adopted marks is the tempting wrong improvement |
| `menu-keyboard-contract` | Every `role="menu"` calls `useMenuKeys`, and a file with two menus wires both. The role is a PROMISE - a screen reader that meets it switches its user into menu navigation and tells them to arrow - and eleven menus made it while two handled an arrow key, which is worse than not using the role at all. A source scan rather than more e2e because it is a POPULATION problem: `popover-focus` reaches three triggers and eight of the eleven are unreachable from its boot state |
| `mousedown-activation-contract` | No `<button>` is operable by pointer alone. Enter and Space dispatch a `click`, never a `mousedown`, so `onMouseDown` with no `onClick` is a Level A keyboard failure that looks entirely ordinary in review - nine shipped, the whole review export menu among them. Note the parser: `<button([^>]*)>` is WRONG for JSX because an arrow function contains a `>`, which reads a two-handler button as mousedown-only; the scan tracks brace depth so a `>` only closes the tag at depth zero |
| `resize-handle-contract` | Every `cp-resize-handle` splitter is focusable, answers keys, and SAYS so in its title. Four exist; two carried tabIndex and arrows and two were pointer-only while declaring `role="separator"` with an aria-label - a control advertised to a screen reader with no way to work it (2.1.1 Level A, and 2.5.7 in WCAG 2.2). A source scan because none of the four renders in the two states `target-size.spec.ts` reaches |
| `segmented-choice-contract` | Every `.cp-segmented` picker is a NAMED radiogroup whose segments say which is chosen. Seven rendered the selection as a CSS class and a sliding `::before` pill, so every button announced identically and nothing said the group belonged together. Scoped per container, which is the whole trick: counting `aria-checked` file-wide let a `role="switch"` elsewhere in Sidebar mask the mutation, and a guard that counts the right things in the wrong scope cannot fail |
| `review-writer-contract` | Every exported `lib/review.ts` updater is reachable from production, or carries a stated reason that must still match. Found the source-level VERDICT: `setStatus` has no writer anywhere, so every source reads Pending for ever and the Markdown a client receives says Pending about a cut the room approved - while the op, the LWW arm, the undo inverse, three chips and an export line all exist. The scan is IMPORT-AWARE and alias-aware, and both halves matter: a bare `setStatus(` search finds three unrelated useState setters, and recording a local alias instead of the exported name makes an aliased import read as no caller |
| `store-problem-contract` | Every store failure reporter has a subscriber in App that NOTIFIES, not just logs. Three exist - a review note that never reached disk, a file from a newer build, and localStorage full - and each is half a mechanism: a reporter whose subscriber is deleted keeps firing perfectly into an empty Set, which is the state all three were built to escape. Each replaced a console.warn, in an app whose WKWebView console needs Safari's inspector attached to read |
| `override-persistence-contract` | Speaker-override emptiness is DERIVED from the object, never hand-listed. `SpeakerOverrides` has eight sub-maps and the check tested five - and the branch it guards calls `removeItem`, so a user whose only edit was a per-cue reassignment (the layer that separates two people the diarizer merged) was measured as having changed nothing and had their overrides DELETED. Not failed-to-save: removed |
| `quit-flush-contract` | Every debounced Documents store registers a `pagehide` flush, from the STORE rather than a component. Only review-store did. cast-store's flush docstring said "called before the window closes" while its only caller was a React unmount, which does not run on close; web-collection had one nobody called on quit; projects had no flush at all. Stores matched by SHAPE (a debounce timer plus a disk write), so a fifth is covered without anyone remembering |
| `undo-redo-fidelity-contract` | Redo restores what UNDO REMOVED, not what the action originally added. The inverse of an `add` is a `del`, which cascades to replies - and the inverse is computed eagerly at post time, when there are none. In a session a peer replies seconds later, cmd+Z relays a del that takes their words off every machine, and redo replays only the captured comment. Also pins that the inverse stays EAGER: snapshotting a doc per entry is the tempting fix and pins fifty ReviewDocs, the leak eagerness exists to avoid |
| `placeholder-contrast-contract` | Every `::placeholder` colour clears 4.5:1, computed from the tokens so a palette retune fails it too. Placeholder text is text, and NEITHER rendered contrast sweep can see it: both build their population from elements with child text nodes and read `getComputedStyle(el).color`, which never reports a pseudo-element. Two placeholders sat below AA behind that, one of them inside the dialog `contrast.spec.ts` opens on purpose |
...
| `list-custom-column` | The column flow driven from the chair: right-click a heading, New Column, type, Enter, and a heading plus a cell in every row appears, aligned to the pixel; the cell opens an editor on a selected row; rename and delete keep the grid honest; Size sorts largest-first like Finder; a divider drag resizes by what was dragged. The model was unit-tested and the empty cell was still zero pixels tall, because the row centres its items and nothing measured the target |
| `list-columns` | The three list views' cells sit under their headings, measured in rendered pixels, and a column carries its cells when moved, hidden or resized. Order and track count were already checked and passed while every column on the web shelf sat 40px left of its heading: the rows reserved a gutter the header did not. Only x positions could see it |
| `no-bleed` | In a non-wrapping flex row, no in-flow child paints over its sibling. The class behind four separate reports in one sitting: a row where every child can shrink and none is pinned. Absolutely positioned children are skipped, since a badge is meant to sit over things. Blind to a live session and to a resolved comment, which is where the reports came from |
