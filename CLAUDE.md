...
| `rust-panic-contract` | No .unwrap()/.expect()/panic! in production Rust (one allowlisted, with a stated reason). A panic in a command handler never resolves the invoke and poisons any Mutex it held |
