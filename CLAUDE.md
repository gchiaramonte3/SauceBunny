...
| `session-identity-contract` | A screening's id is never the iroh join ticket. `SessionState.code`It carries the host node id and its LAN/reflexive socket addresses, and was being persisted into ~/Documents and keyed into index.json |
