...
//   1. mediabunny extractFrameAsBlob — in-process WebCodecs decode (~20-80ms,
//      no subprocess), yields a blob: object URL.
//   2. generate_local_thumbnail — the ffmpeg fallback for codecs WebCodecs
//      can't decode; has its own (path, mtime)-keyed disk cache, yields an
//      asset:// URL via convertFileSrc.
// Total failure marks the path bad for the session → placeholder, no retry
// loop. Duration probing is deliberately out of scope (v1): frame 0 is fine.
// ════════════════════════════════════════════════════════════════════════
...
async function loadThumbnail(path: string): Promise<string | null> {
  const blob = await extractFrameAsBlob(path, 0, { maxWidth: 480, quality: 0.8 });
  if (blob) return URL.createObjectURL(blob);
  const out = await invoke<string>("generate_local_thumbnail", {
    args: { input_path: path, duration_seconds: null },
  });
