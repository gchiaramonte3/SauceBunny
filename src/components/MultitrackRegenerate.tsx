...
export function MultitrackRegenerate({ owner, generated = false, initial, models, parakeetReady, onClose, onStart }: {
  owner: string; generated?: boolean; initial: MultitrackModelChoice; models: WhisperModel[]; parakeetReady: boolean;
  ...
  return createPortal(<div className="cp-modal-scrim" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div ref={dialog} className="cp-multitrack-settings" role="dialog" aria-modal="true" aria-label={`${action} ${owner}`} tabIndex={-1} onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); onClose(); } }}>
