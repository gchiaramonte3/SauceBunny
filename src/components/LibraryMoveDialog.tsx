 ...
 */
export function LibraryMoveDialog({ paths, folders, onMove, onClose }: {
  ...
  return createPortal(
    <div className="cp-rowmenu-scrim" onPointerDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      ...
      <div ref={dialogRef} tabIndex={-1} className="cp-rowmenu-dialog" role="dialog" aria-modal="true" aria-label="Move to folder">
        ...
        {folders.length === 0 ? (
          // Said plainly rather than showing an empty box. There is nowhere to
          // move to because this folder has no subfolders, and "New folder" in
          // the bar above is the answer.
