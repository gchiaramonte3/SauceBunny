...
it("shows an honest failed-run summary and keeps the details behind the info button", () => {
  ...
  expect(screen.queryByText(/Speech cue is outside/)).toBeNull();
  fireEvent.click(screen.getByRole("button", { name: "Transcript info" }));
  const dialog = screen.getByRole("dialog", { name: "Transcript info" });
  expect(within(dialog).getByText(/Speech cue is outside/)).toBeTruthy();
  expect(within(dialog).getByText(/Saved sequences/)).toBeTruthy();
