...
export function MultitrackSettings({ document, waveformErrors, onClose }: { document: AafDocument; waveformErrors: Record<string, string>; onClose: () => void }) {
  ...
  return createPortal(<div className="cp-modal-scrim" onMouseDown={onClose}>
    <div ref={ref} className="cp-multitrack-settings" role="dialog" aria-modal="true" aria-label="Multitrack settings" tabIndex={-1} onMouseDown={(event) => event.stopPropagation()} onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); onClose(); } }}>
      ...
      <details><summary>Avid marker exports</summary><p>Markers use original sequence timecode and the mic owner's name. One point is placed at each timed passage's starting frame on V1. Speech timing is estimated by the transcription engine, not verified word alignment. Same-frame passages for one owner are combined.</p><p>Untimed passages stay in text exports; they are not assigned invented marker positions. Avid files by person saves one file per named owner with timed text, preserving existing files.</p><p>In Avid, open the matching sequence in the Record Monitor, open the Markers window, then right-click and choose Import Markers. Import the exported .txt file.</p></details>
