...
/**
 * Build the print-to-PDF document for a transcript.
 *
 * Pulled out of TranscriptViewer because of WHERE this string ends up: it is
 * not written to a file, it is `document.write`n into a live iframe and
 * printed. Everything interpolated into it therefore executes if it is not
 * escaped, and transcript text is not necessarily the user's own writing -
 * captions can be downloaded alongside a video, so the words in a cue can come
 * from whoever published it.
 *
 * Every interpolation was already escaped. What was missing was any way to
 * PROVE that, or to notice when a later edit adds a sixth interpolation and
 * forgets. A pure function returning a string can be handed a hostile speaker
 * name and asked what it did with it; the same code inline in a 2,200-line
 * component cannot.
 *
 * The document deliberately carries no script of its own, so the only script
 * that could ever run in that iframe is one that arrived through here.
 */
export function buildTranscriptPrintDoc(
  turns: readonly Turn[],
  baseFilename: string,
  nameFor: (turnIndex: number, speaker: string | null) => string,
  timeFor: (seconds: number) => string,
  note?: string,
  rangeFor?: (start: number, end: number) => string,
): string {
  const turnsHtml = turns.map((turn, ti) => {
    const name = escapeHtml(nameFor(ti, turn.speaker));
    const time = escapeHtml(rangeFor ? rangeFor(turn.start, turn.end) : timeFor(turn.start));
  ...
  const title = escapeHtml(baseFilename);
  return `<!doctype html>
<html><head><meta charset="utf-8"><title>${title}</title>
<style>
  @page { margin: 0.75in; }
  body { font: 11pt/1.5 Georgia, 'Iowan Old Style', serif; color: #111; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  .body { padding-left: 0; overflow-wrap: anywhere; white-space: pre-wrap; }
  .name, h1 { overflow-wrap: anywhere; }
  .head { gap: 12pt; }
  .time { flex: none; }
</style></head>
<body>
  ...
  ${note ? `<p class="meta" style="white-space: pre-wrap">${escapeHtml(note)}</p>` : ""}
  ${turnsHtml}
</body></html>`;
}
