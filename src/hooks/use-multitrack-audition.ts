...
export function useMultitrackAudition(document: AafDocument, active: boolean) {
  ...
  const [solo, setSolo] = useState(new Set<string>()), [mute, setMute] = useState(new Set<string>());
  ...
  const getEngine = useCallback(() => {
    ...
    if (!engine.current && current.active) {
      engine.current = new MultitrackAudio(current.document.id, sequenceFps(current.document.manifest), current.document.manifest.duration_frames,
        audibleTracks(current.document.manifest.tracks.map((track) => track.id), current.solo, current.mute), setState);
      engine.current.setLevel(current.volume, current.muted); engine.current.setScrubbing(current.scrubbing);
    }
    return engine.current;
  }, []);
  useEffect(() => () => { engine.current?.close(); engine.current = null; }, [document.id]);
  useEffect(() => { if (active) void getEngine()?.warm(latest.current.frame); }, [active, document.id, getEngine]);
  ...
  const trackKey = document.manifest.tracks.map((track) => track.id).join("|");
  useEffect(() => { engine.current?.setTracks(audibleTracks(trackKey.split("|"), solo, mute)); }, [trackKey, solo, mute]);
  ...
  return { ...state, playing: state.rate !== 0, solo, mute, toggleSolo: (id: string) => toggleSet(setSolo, id), toggleMute: (id: string) => toggleSet(setMute, id),
