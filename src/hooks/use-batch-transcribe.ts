...
export function useBatchTranscribe(
    ...
    try {
      for (;;) {
        ...
        try {
          ...
          // Everything after the listener exists releases it on every exit: a
          // Stop re-check that breaks, a spawn that rejects, or a failed done.
          // Only the success path used to, so each failed or cancelled file
          // left one transcript-done listener attached for the app's lifetime.
          try {
  ...
  return { state, progress: batchProgress(state), start, cancel, clear };
}
