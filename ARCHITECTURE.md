...
- **Writers** — the active player's `onTimeUpdate`, every seek/step/reset path in `App.tsx`, and `PanelApp` (the floating panel is a separate webview with its own store instance; it mirrors the snapshot's playhead in).
...
- **One low-frequency mirror** — the popped-out panel can't subscribe across webviews, so while (and only while) a panel is detached, App keeps a 4 Hz mirror that feeds the playhead into the `panel:state` snapshot as data.
